...
def normalize(dd_func_archive, amosaParams):
    ...
    for i in range(amosaParams.i_no_offunc):
        # Normalize shift
        for j in range(len(dd_func_archive)):
            dd_func_archive[j][i] = dd_func_archive[j][i] - d_normalize_shift[i]
        # Normalize scale
        for j in range(len(dd_func_archive)):
...
def associate(
    normalized_dd_func_archive, dd_func_archive, dd_archive, refPoints, associationList
):
    ...
    for i in range(len(dd_func_archive)):
        ...
        for j in range(len(refPoints)):
            ...
            if nDistance < minDistance:
                ...
                minDistanceIndex = j
        associationList[minDistanceIndex].append(
            [normalized_dd_func_archive[i], dd_func_archive[i], dd_archive[i]]
        )
...
def niching(dd_func_archive, dd_archive, associationList, refPoints, i_hardl):
    ...
    while len(clustered_dd_archive) >= i_hardl:
        ...
        for i in range(len(associationList[maxAssocIndex])):
            distance = cost(
                associationList[maxAssocIndex][i][0], refPoints[maxAssocIndex]
            )
        ...
        # removing the point
        try:
            clustered_dd_func_archive.remove(
                associationList[maxAssocIndex][maxDistanceIndex][1]
            )
        except ValueError:
            print(ValueError)
            print("----------")
...
def removeDominated(amosaParams):
    # Remove dominated points depending on temperature
    # Step 1 : flag mark dominated solutions
    i_flag = [0] * len(amosaParams.dd_archive)
    for i in range(len(amosaParams.dd_archive)):
        if i_flag[i] == 0:
            for j in range(i + 1, len(amosaParams.dd_archive)):
                if i_flag[i] == 1:
                    break
                count0 = 0
                count1 = 0
                for h in range(amosaParams.i_no_offunc):
                    if (
                        amosaParams.dd_func_archive[i][h]
                        >= amosaParams.dd_func_archive[j][h]
                    ):
                        count0 = count0 + 1
                    elif (
                        amosaParams.dd_func_archive[i][h]
    ...
    old_archive = copy.deepcopy(amosaParams.dd_archive)
    old_func_archive = copy.deepcopy(amosaParams.dd_func_archive)
    amosaParams.dd_archive = []
    amosaParams.dd_func_archive = []

    # Storing only non-dominated solutions
    for i in range(len(old_archive)):
        if i_flag[i] == 0:
            amosaParams.dd_archive.append(old_archive[i])
            amosaParams.dd_func_archive.append(old_func_archive[i])
